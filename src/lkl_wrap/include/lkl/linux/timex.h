...
struct __lkl__kernel_timex_timeval {
	__lkl__kernel_time64_t       tv_sec;
	long long		tv_usec;
};

struct __lkl__kernel_timex {
	unsigned int modes;	/* mode selector */
	int :32;            /* pad */
	...
	long long tolerance;/* clock frequency tolerance (ppm)
				   * (read only)
				   */
	struct __lkl__kernel_timex_timeval time;	/* (read only, except for LKL_ADJ_SETOFFSET) */
