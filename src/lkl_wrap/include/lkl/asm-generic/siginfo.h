...
/*
 * Be careful when extending this union.  On 32bit lkl_siginfo_t is 32bit
 * aligned.  Which means that a 64bit field or any other field that
 * would increase the alignment of lkl_siginfo_t will break the ABI.
 */
union __lkl__sifields {
	/* kill() */
	struct {
...
#define __LKL__ADDR_BND_PKEY_PAD  (__alignof__(void *) < sizeof(short) ? \
		...
		union {
			...
			/* used when si_code=LKL_TRAP_PERF */
			struct {
				unsigned long _data;
				__lkl__u32 _type;
				__lkl__u32 _flags;
...
#define __LKL__SIGINFO 			\
struct {				\
	int si_signo;			\
	int si_errno;			\
	int si_code;			\
	union __lkl__sifields _sifields;	\
}
#else
#define __LKL__SIGINFO 			\
struct {				\
...
typedef struct lkl_siginfo {
	union {
		__LKL__SIGINFO;
		int _si_pad[LKL_SI_MAX_SIZE/sizeof(int)];
	};
...
#define lkl_si_perf_data	_sifields._sigfault._perf._data
#define lkl_si_perf_type	_sifields._sigfault._perf._type
#define lkl_si_perf_flags	_sifields._sigfault._perf._flags
...
#define LKL_SEGV_MTEAERR	8	/* Asynchronous ARM MTE error */
#define LKL_SEGV_MTESERR	9	/* Synchronous ARM MTE exception */
#define LKL_NSIGSEGV	9
...
#define LKL_TRAP_PERF	6	/* perf event with sigtrap=1 */
#define LKL_NSIGTRAP	6
...
/*
 * Flags for lkl_si_perf_flags if LKL_SIGTRAP si_code is LKL_TRAP_PERF.
 */
#define LKL_TRAP_PERF_FLAG_ASYNC (1u << 0)
...
#define LKL_SYS_USER_DISPATCH 2	/* syscall user dispatch triggered */
#define LKL_NSIGSYS		2

/*
 * SIGEMT si_codes
 */
#define LKL_EMT_TAGOVF	1	/* tag overflow */
#define LKL_NSIGEMT		1
