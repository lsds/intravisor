...
//      printf("%p starting thread[%d]: [%lx -- %lx], func = %p, arg = %p\n",me, me->id, (long)me->stack, (long)me->stack+me->stack_size, me->func, me->arg);
	...
	void *__capability ccap = codecap_create((void *) me->sbox->cmp_begin, (void *) me->sbox->cmp_end, me->sbox->cr);
	void *__capability dcap = datacap_create((void *) me->sbox->cmp_begin, (void *) me->sbox->cmp_end, me->sbox->cr);
	...
	if(!cs->call_lock.predicate)
		pthread_cond_wait(&cs->call_lock.cond, &cs->call_lock.lock);
...
//printf("HOST_CALL: CARRIE_THREAD %d %p %p\n", tmp, f, getSP());
	...
	pthread_mutex_lock(&cs->call_lock2.lock);
	if(!cs->call_lock2.predicate)
		pthread_cond_wait(&cs->call_lock2.cond, &cs->call_lock2.lock);
...
void host_cap_wait(int id) {
	...
	if(!cs->predicate)
		pthread_cond_wait(&cs->cond, &cs->lock);
	cs->predicate = false;
