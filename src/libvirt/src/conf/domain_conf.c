...
//Intravisor
    VIR_FREE(def->os.disk);
    VIR_FREE(def->os.callback_out);
    VIR_FREE(def->os.base);
    VIR_FREE(def->os.size);
    VIR_FREE(def->os.begin);
    VIR_FREE(def->os.end);
...
//def->os.type == VIR_DOMAIN_OSTYPE_UML
    ...
    if (def->os.type == VIR_DOMAIN_OSTYPE_UML) {
        ...
        def->os.disk = virXPathString("string(./os/disk[1])", ctxt);
        def->os.cmdline = virXPathString("string(./os/cmdline[1])", ctxt); //args
        def->os.callback_out = virXPathString("string(./os/callback_out[1])", ctxt);
        def->os.base = virXPathString("string(./os/base[1])", ctxt);
        def->os.size = virXPathString("string(./os/size[1])", ctxt);
        def->os.begin = virXPathString("string(./os/begin[1])", ctxt);
        def->os.end = virXPathString("string(./os/end[1])", ctxt);
    }
