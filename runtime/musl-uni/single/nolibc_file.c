...
ssize_t writev(int fd, const struct iovec *iov, int iovcnt) {
	return my_writev(fd, iov, iovcnt);
}

ssize_t readv(int fd, const struct iovec *iov, int iovcnt) {
	return my_readv(fd, iov, iovcnt);
}
...
//      printf("%s\n", __func__);
	if(fd < 3)
		return host_write_out(buf, count);
...
static char fprintf_buf[4096];
int fprintf(FILE * __restrict f, const char *__restrict fmt, ...) {
	int ret;
	va_list ap;
	va_start(ap, fmt);
//    ret = vfprintf(stdout, fmt, ap);
	ret = vsnprintf(fprintf_buf, 4096, fmt, ap);
	va_end(ap);

	host_write_out(fprintf_buf, ret);
}
...
FILE *fdopen(int fd, const char *mode) {
	...
	if(!(f = my_malloc(sizeof *f + UNGET + BUFSIZ)))
		return 0;
