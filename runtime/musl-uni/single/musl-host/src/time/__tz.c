...
static const unsigned char *zi, *trans, *index2, *types, *abbrevs, *abbrevs_end;
...
static void do_tzset()
{
	...
	if (map) {
		...
		index2 = trans + (zi_read32(trans-12) << scale);
		types = index2 + zi_read32(trans-12);
...
static size_t scan_trans(long long t, int local, size_t *alt)
{
	...
	size_t a = 0, n = (index2-trans)>>scale, m;
	...
	while (n > 1) {
		...
		if (local) off = (int32_t)zi_read32(types + 6 * index2[m-1]);
		if (t - off < (int64_t)x) {
			n /= 2;
		} else {
			a = m;
	...
	/* First and last entry are special. First means to use lowest-index2
	 * non-DST type. Last means to apply POSIX-style rule if available. */
	n = (index2-trans)>>scale;
	if (a == n-1) return -1;
	if (a == 0) {
		x = zi_read32(trans + (a<<scale));
		if (scale == 3) x = x<<32 | zi_read32(trans + (a<<scale) + 4);
		else x = (int32_t)x;
		if (local) off = (int32_t)zi_read32(types + 6 * index2[a-1]);
		if (t - off < (int64_t)x) {
	...
	if (alt) {
		if (a && types[6*index2[a-1]+4] != types[6*index2[a]+4])
			*alt = index2[a-1];
		else if (a+1<n && types[6*index2[a+1]+4] != types[6*index2[a]+4])
			*alt = index2[a+1];
		else
			*alt = index2[a];
	}
...
int days_in_month(int m, int is_leap)
{
