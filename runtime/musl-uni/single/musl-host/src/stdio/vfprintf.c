...
static int fmt_cap(FILE *f, const void *cap) {
	...
	*--z = ']';
	...
	size_t upper_bound = lower_bound + __builtin_cheri_length_get(cap);
...
value:
	if ((uintmax_t)cap == 0) {
		*--z = '0';
		...
		z = fmt_x((uintmax_t)cap, z, 32);
	}
	*--z = 'x';
	*--z = '0';
	size_t length = (size_t)buf + sizeof(buf) - (size_t)z;
	out(f, z, length);
	return length;
}
#endif
