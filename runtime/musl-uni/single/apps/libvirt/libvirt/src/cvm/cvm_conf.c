/*
 * cvm_conf.c: CVM driver configuration, based on old UML driver
 *
...
static int
cvmConnectTapDevice(virDomainDefPtr vm,
...
static char *
cvmBuildCommandLineNet(virConnectPtr conn,
                       virDomainDefPtr vm,
                       ...
                       int idx)
{
    ...
    case VIR_DOMAIN_NET_TYPE_NETWORK:
    {
        ...
        if (cvmConnectTapDevice(vm, def, bridge) < 0) {
            VIR_FREE(bridge);
            goto error;
        }
...
static char *
cvmBuildCommandLineChr(virDomainChrDefPtr def,
                       const char *dev,
                       virCommandPtr cmd)
{
    char *ret = NULL;
 ...
 */
static char *cvmNextArg(char *args)
{
    int in_quote = 0;
...
/*
 * Constructs a argv suitable for launching cvm with config defined
                                  ...
                                  virDomainObjPtr vm)
{
    ...
    virCommandAddArgPair(cmd, "cvm_dir", driver->monitorDir);
    ...
    for (i = 0; i < vm->def->nnets; i++) {
        char *ret = cvmBuildCommandLineNet(conn, vm->def, vm->def->nets[i], i);
        if (!ret)
            goto error;
    ...
    for (i = 0; i < CVM_MAX_CHAR_DEVICE; i++) {
        ...
        if (chr)
            ret = cvmBuildCommandLineChr(chr, "con", cmd);
        if (!ret)
    ...
    for (i = 0; i < CVM_MAX_CHAR_DEVICE; i++) {
        ...
        if (chr)
            ret = cvmBuildCommandLineChr(chr, "ssl", cmd);
        if (!ret)
