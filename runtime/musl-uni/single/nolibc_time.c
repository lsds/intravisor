...
//generated by chatgpt, needs check
size_t strftime(char *str, size_t maxsize, const char *format, const struct tm *timeptr) {
#if 0
    size_t i = 0;
    while (*format && i < maxsize - 1) {
        if (*format == '%') {
            format++; // Move to the next character after '%'
            switch (*format) {
                case 'Y': // Year with century as a decimal number
                    i += snprintf(str + i, maxsize - i, "%d", timeptr->tm_year + 1900);
                    break;
                case 'm': // Month as a decimal number (01-12)
                    i += snprintf(str + i, maxsize - i, "%02d", timeptr->tm_mon + 1);
                    break;
                case 'd': // Day of the month as a decimal number (01-31)
                    i += snprintf(str + i, maxsize - i, "%02d", timeptr->tm_mday);
                    break;
                case 'H': // Hour (00-23)
                    i += snprintf(str + i, maxsize - i, "%02d", timeptr->tm_hour);
                    break;
                case 'M': // Minute (00-59)
                    i += snprintf(str + i, maxsize - i, "%02d", timeptr->tm_min);
                    break;
                case 'S': // Second (00-59)
                    i += snprintf(str + i, maxsize - i, "%02d", timeptr->tm_sec);
                    break;
                default:
                    // If the format specifier is not recognized, print the '%' character
                    str[i++] = '%';
                    if (i < maxsize - 1)
                        str[i++] = *format;
            }
