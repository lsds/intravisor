...
extern int lkl_printf(const char *fmt, ...);
...
lkl_printf("tsd = %p, libc.tls_size = %lx, stack = %p\n", tsd, libc.tls_size, stack);
...
//	sd a3, 8(a1)
	...
	long *ts = (long *) ((long) stack - 24);
	ts[0] = start;
	ts[1] = args;
	ts[2] = TP_ADJ(new);

	printf("start = %p, args = %p, ts = %p\n", start, args, ts);
