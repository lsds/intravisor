...
static struct dso *load_library(const char *name, struct dso *needed_by)
{
	...
	lkl_printf("LOAD: %p: %s at %p\n", __builtin_frame_address(0), name, p->base);
	return p;
}
...
long local_cap_store = 0xabbacaca;
...
//	local_cap_store = 0x3e001000; //base + stack offset + 1 page above the bottom
	local_cap_store = aux[AT_UID];
	aux[AT_UID] = -1;
