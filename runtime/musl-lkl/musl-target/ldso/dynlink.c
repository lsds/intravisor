...
//if(lkl_ok && def.sym) lkl_printf("def.sym = %p %p %p %p\n", def.sym, def.dso, def.sym->st_value, sym_val);
		...
		case REL_GOT:
		case REL_PLT:
//if(lkl_ok) lkl_printf("%s:\t%p %p %p %p\n", name, reloc_addr, sym_val, addend, base);
...
{
return;//FIXME, actually we simply don't need this because we cannot give this memory to our allocator
...
//	__pthread_self()->sysinfo = __sysinfo; FIXME
	...
	if(libc.tls_head != 0) {
		__asm__ __volatile__("ebreak");
		lkl_printf("libc.tls_head = %p, die\n", libc.tls_head);
		while(1);
	}

#ifdef SIM
	local_cap_store = aux[AT_BASE] + 0xe000000 + 0x1000; //base + stack offset + 1 page above the bottom
#else
	local_cap_store = 0xe000000 + 0x1000; //base + stack offset + 1 page above the bottom
#endif
extern void init_internal_lkl(void);
	init_internal_lkl();
...
extern void init_network(unsigned long comp_to_mon);
	init_network(aux[AT_BASE]); //beware: networking corrupts memory of the first thread (somewhere near the 2nd argument), when it tries to make a lock (pthread_mutex_trylock '*next')
					//note 2: this might not be a problem anymore, needs investigation
...
extern int lkl_printf(const char *format, ...);
			lkl_printf("%d %s: cannot load %s: %s\n", __LINE__, ldname, argv[0], strerror(errno));
...
//	lkl_ok = 0;
...
#else
		void *ptr = __copy_tls((void*)builtin_tls);
		if ( ptr != self) {
			lkl_printf("%p %p\n", ptr, self);while(1);
			a_crash();
		}
	...
	lkl_printf("ready to start the app: %p\n", (void *)aux[AT_ENTRY]);
