 ...
 */
int cpio_parse_header(struct cpio_header *archive, const char **filename, unsigned long *_filesize, intptr_t ** data, struct cpio_header **next) {
...
//      printf("ptr = %x %x tag = %d base = %x\n", (int) __builtin_cheri_length_get(ptr), (int) ptr, __builtin_cheri_tag_get(ptr), (int) __builtin_cheri_base_get(ptr));
		...
		printf("writing file '%s', ret = %d, size = %d\n", path, wr, size);
		close(fd);
	}
}
