...
bool gpt2_model_load(const std::string & fname, gpt2_model & model, gpt_vocab & vocab) {
    ...
    int fd = open("/ggml-model.bin", O_RDONLY, 0666);
    printf("fd = %d\n", fd);
    if( fd < 0 ) {
...
//	    return false;
...
#else
    {
        ...
        if (magic != GGML_FILE_MAGIC) {
            printf("%s: invalid model file '%s' (bad magic)\n", __func__, fname.c_str());
            return false;
        } else
    	    printf("magic is ok\n");
    }
    ...
    // load hparams
    {
        ...
        read(fd, (char *) &hparams.n_vocab, sizeof(hparams.n_vocab));
        read(fd, (char *) &hparams.n_ctx,   sizeof(hparams.n_ctx));
        read(fd, (char *) &hparams.n_embd,  sizeof(hparams.n_embd));
        read(fd, (char *) &hparams.n_head,  sizeof(hparams.n_head));
        read(fd, (char *) &hparams.n_layer, sizeof(hparams.n_layer));
        read(fd, (char *) &hparams.ftype,   sizeof(hparams.ftype));
    ...
    {
        int32_t n_vocab = 0;
        read(fd, (char *) &n_vocab, sizeof(n_vocab));

        if (n_vocab != model.hparams.n_vocab) {
            printf("%s: invalid model file '%s' (bad vocab size %d != %d)\n",
                    __func__, fname.c_str(), n_vocab, model.hparams.n_vocab);
            return false;
        }
...
#else
            int rr = read(fd, reinterpret_cast<char *>(&n_dims), sizeof(n_dims));
            if(rr == 0)
        	    break;

            read(fd, reinterpret_cast<char *>(&length), sizeof(length));
            read(fd, reinterpret_cast<char *>(&ttype),  sizeof(ttype));
...
#else
                read(fd, reinterpret_cast<char *>(&ne[i]), sizeof(ne[i]));
#endif
                nelements *= ne[i];
            }
...
#else
            int ret = read(fd, reinterpret_cast<char *>(tensor->data), ggml_nbytes(tensor));
//            printf("%d ret = %d\n", __LINE__, ret);
//            dm(tensor->data, ggml_nbytes(tensor));
//            printf("-------\n");
...
//		printf("logits.data() = %p, logits.size() = %lx, n_vocab = %lx\n", logits.data(), logits.size(), n_vocab);
                id = gpt_sample_top_k_top_p(vocab, logits.data() + (logits.size() - n_vocab), top_k, top_p, temp, rng);
//		printf("ret id = %d (%d %f %f)\n", id, top_k, top_p, temp);
