...
#ifdef INTRAVISOR
//the lock is passed via _clone, and it should be unlocked in SYS_exit
	__tl_unlock();
#endif
...
#ifdef INTRAVISOR
	__asm__ __volatile__("msr ctpidr_el0, %0;" :: "C"(args->new) : "memory");
#endif
...
#ifdef INTRAVISOR
void *mmap_wrap(void *addr, size_t len, int prot, int flags, int fd, off_t offset) {
	if((addr != 0) || (offset != 0) || (fd != -1)) {
		printf("something is wrong at %s %d\n", __FILE__, __LINE__);
		abort();
	}

	return memalign(4096, len);
}

int munmap_wrap(void *addr, size_t len) {
//	free(addr);
	return 0;
}
#endif
...
int __pthread_create(pthread_t *restrict res, const pthread_attr_t *restrict attrp, void *(*entry)(void *), void *restrict arg)
{
	...
	if (!tsd) {
		if (guard) {
#ifdef INTRAVISOR
			map = mmap_wrap(0, size, PROT_NONE, MAP_PRIVATE|MAP_ANON, -1, 0);
#else
...
#ifdef __CHERI_PURE_CAPABILITY__
#ifdef INTRAVISOR
			map = mmap_wrap(0, size, PROT_READ|PROT_WRITE, MAP_PRIVATE|MAP_ANON, -1, 0);
#else
...
#ifdef INTRAVISOR
		tsd = mmap_wrap(0, ROUND(__pthread_tsd_size + libc.tls_size), PROT_READ|PROT_WRITE, MAP_PRIVATE|MAP_ANON, -1, 0);
#else
