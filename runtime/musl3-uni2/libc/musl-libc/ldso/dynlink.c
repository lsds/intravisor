	...
	for (; rel_size; rel+=stride, rel_size-=stride*sizeof(size_t)) {
		...
		case REL_CAPINIT:
			{
				...
				*reloc_addr = cap;
#ifdef INTRAVISOR
				printf("CAPINIT: %lx --> %lx, tag %d \n", cap, reloc_addr, __builtin_cheri_tag_get(cap));
				if(!__builtin_cheri_tag_get(cap)) while(1);
#endif
			}
#else
		...
		case REL_RELATIVE: {
			...
			*/
			...
			if(!__builtin_cheri_tag_get(cap_rx)) {
				printf("cannot bounds_set_exact:\t%p:%lx\n", v_addr, len);
				cap_rx = __builtin_cheri_bounds_set(v_addr, len);
			}

			if(!__builtin_cheri_tag_get(cap_rw))
				cap_rw = __builtin_cheri_bounds_set(set_rw_cap(dso, v_addr), len);
...
#else
	lseek(fd, 0, SEEK_SET);
	map = aligned_alloc(PAGE_SIZE, map_len);
	printf("ldso: Loading ELF:\t allocated %p, +%lx\n", map, map_len);
	memset(map, 0xaa, map_len);
	read(fd, map, map_len);
#endif
	dso->map = map;
	dso->map_len = map_len;
...
done_mapping:
#if 0
//I do not understand this. base seems to be at the same RW and RX, which should not be possible
	dso->base = base;
#ifdef __CHERI_PURE_CAPABILITY__
	dso->rw_capability = __builtin_cheri_perms_and(base, __CHERI_CAP_PERMISSION_GLOBAL__ | READ_CAP_PERMS | WRITE_CAP_PERMS);
#endif
#else
	dso->base = __builtin_cheri_address_set(__builtin_cheri_program_counter_get(), base);
	dso->rw_capability = __builtin_cheri_perms_and( __builtin_cheri_address_set(__builtin_cheri_global_data_get(), base), __CHERI_CAP_PERMISSION_GLOBAL__ | READ_CAP_PERMS | WRITE_CAP_PERMS);
#endif
...
#ifdef INTRAVISOR
	printf("ldso.phdr = %lx, base = %lx/%lx %lx\n", ldso.phdr, base, ldso.base, ehdr->e_phoff);
	printf("Version: %p Entry = %p Machine %lx Type = %lx\n", ehdr->e_version, ehdr->e_entry, ehdr->e_machine, ehdr->e_type);
#endif
